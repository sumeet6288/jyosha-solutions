...
curl https://install-preview-9.preview.emergentagent.com/api/chatbots

# This works (with valid token)
curl -H "Authorization: Bearer YOUR_VALID_TOKEN" https://install-preview-9.preview.emergentagent.com/api/chatbots
```
...
fetch('https://install-preview-9.preview.emergentagent.com/api/chatbots')
  .then(r => r.json())
...
```bash
curl -X POST https://install-preview-9.preview.emergentagent.com/api/chatbots \
...
for i in {1..70}; do
  curl https://install-preview-9.preview.emergentagent.com/api/ &
done
wait
...
```bash
curl -I https://install-preview-9.preview.emergentagent.com/
