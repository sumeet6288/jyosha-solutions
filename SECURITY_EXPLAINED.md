...
curl https://depstream.preview.emergentagent.com/api/chatbots

# This works (with valid token)
curl -H "Authorization: Bearer YOUR_VALID_TOKEN" https://depstream.preview.emergentagent.com/api/chatbots
```
...
fetch('https://depstream.preview.emergentagent.com/api/chatbots')
  .then(r => r.json())
...
```bash
curl -X POST https://depstream.preview.emergentagent.com/api/chatbots \
...
for i in {1..70}; do
  curl https://depstream.preview.emergentagent.com/api/ &
done
wait
...
```bash
curl -I https://depstream.preview.emergentagent.com/
